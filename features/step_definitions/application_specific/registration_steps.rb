...
Если(/^правильно заполняет ОГРНИП и телефон и переходит к следующему шагу$/) do
...
То(/^выбирает тарифный план (Квартал|Год?)$/) do |plan|
  step %Q(в форме тариф Квартал имеется поле ввода "Введите промокод")
  step %Q(в форме тариф Год имеется поле ввода "Введите промокод")
  VCR.use_cassette(Presets.cassette) do
...
Если(/^проверяет свои ОГРНИП, телефон и имя, вводит пароль и переходит к следующему шагу$/) do
